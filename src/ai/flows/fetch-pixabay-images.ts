...
export type FetchPixabayImagesInput = z.infer<typeof FetchPixabayImagesInputSchema>;
...
export type FetchPixabayImagesOutput = z.infer<typeof FetchPixabayImagesOutputSchema>;

export async function fetchPixabayImages(input: FetchPixabayImagesInput): Promise<FetchPixabayImagesOutput> {
...
const fetchPixabayImagesFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!apiKey) {
      console.error('Pixabay API key is not set in .env file (PIXABAY_API_KEY)');
    ...
    try {
      console.log(`Fetching images from Pixabay for query: ${query}, count: ${count}`);
      ...
      if (response.data && response.data.hits) {
        ...
        const validatedImages = response.data.hits.map((hit: any) => {
            ...
            if (hit.id && hit.webformatURL && hit.tags && hit.user) {
                return {
                    ...
                    user_id: hit.user_id,
                    user: hit.user,
                    userImageURL: hit.userImageURL || `https://placehold.co/32x32.png`, // Fallback for missing userImageURL
                };
            }
            return null; 
        }).filter((img: PixabayImage | null): img is PixabayImage => img !== null);


        return { images: validatedImages as PixabayImage[] };
      } else {
        console.warn('No hits found in Pixabay response or unexpected data structure.');
        return { images: [] };
      }
    } catch (error: any) {
