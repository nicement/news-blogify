...
import { Loader2, Newspaper, Brain, Sparkles, ImagePlus, Edit3, Bot, FileText, Code, ExternalLink } from "lucide-react";
...
export default function NewsBlogifyPage() {
  ...
  const [selectedKeywordItem, setSelectedKeywordItem] = useState<NaverNewsKeywordItem | null>(null);
  ...
  const [sourceArticleTitleForDisplay, setSourceArticleTitleForDisplay] = useState<string | null>(null);
  ...
  const handleFetchKeywords = async () => {
    ...
    try {
      ...
      if (result.keywords && result.keywords.length > 0) {
        ...
        toast({ title: "키워드 로드 완료", description: `네이버 뉴스 랭킹 키워드 ${result.keywords.slice(0,10).length}개가 로드되었습니다. (현재는 플레이스홀더 데이터입니다.)` });
      } else {
        toast({ title: "키워드 없음", description: "네이버 뉴스에서 키워드를 가져오지 못했습니다. 스크래핑 로직을 확인하거나 잠시 후 다시 시도해주세요.", variant: "destructive" });
      }
    } catch (error) {
      console.error("Error fetching Naver news keywords:", error);
      toast({ title: "오류 발생", description: "키워드 로드 중 오류가 발생했습니다. 서버 측 스크래핑 로직을 확인해주세요.", variant: "destructive" });
    } finally {
  ...
  const handleKeywordSelect = async (keywordItem: NaverNewsKeywordItem) => {
    ...
    try {
      ...
      toast({ title: "오류 발생", description: "초안 생성 중 오류가 발생했습니다. 서버 측 스크래핑 및 AI 생성 로직을 확인해주세요.", variant: "destructive" });
      setBlogContent("초안 생성에 실패했습니다. 실제 기사 내용을 가져오는 데 실패했거나 AI 생성 중 문제가 발생했을 수 있습니다. 서버 로그를 확인해주세요.");
    } finally {
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col items-center p-4 md:p-8 font-sans">
      <header className="w-full max-w-4xl mb-8 text-center">
        ...
        <p className="text-muted-foreground mt-2 text-lg">
          네이버 뉴스 랭킹 키워드로 손쉽게 블로그 포스트를 작성하세요. 실제 기사 기반 초안 생성 및 형식 변환 기능 제공!
        </p>
      </header>
        ...
        {!selectedKeywordItem ? (
          <Card className="shadow-xl">
            <CardHeader>
              ...
              <CardDescription>
                가장 인기있는 뉴스 키워드(기사 제목)를 확인하고 블로그 작성을 시작하세요. 선택한 기사 원문을 기반으로 초안이 작성됩니다. 
                <span className="font-semibold text-destructive"> (주의: 현재 스크래핑 로직은 플레이스홀더이며, 실제 구현이 필요합니다.)</span>
              </CardDescription>
            </CardHeader>
            <CardContent>
              {keywords.length === 0 && !isLoadingKeywords && (
                <Button onClick={handleFetchKeywords} className="w-full text-lg py-6 bg-accent hover:bg-accent/90 text-accent-foreground">
              ...
              {keywords.length > 0 && (
                <div className="space-y-2">
                  {keywords.map((kwItem) => (
                    ...
                    >
                      <span className="font-semibold mr-2 text-primary">{kwItem.rank}.</span>
                      <span className="truncate flex-grow">{kwItem.keyword}</span>
                       <a
          ...
          <Card className="shadow-xl">
            <CardHeader>
              ...
              <CardDescription className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div className="truncate">
                  선택된 기사: <span className="font-semibold text-primary" title={selectedKeywordItem.keyword}>{selectedKeywordItem.keyword}</span>
            ...
            <CardContent className="space-y-4">
              ...
              {isLoadingDraft ? (
                <div className="flex flex-col justify-center items-center h-60 border border-dashed rounded-md">
                  ...
                  <p className="mt-4 text-lg text-muted-foreground">"{selectedKeywordItem?.keyword}" 기사 기반 초안 생성 중...</p>
                  <p className="text-sm text-muted-foreground">(실제 기사 내용을 가져오고 AI가 처리하는 중입니다... 현재는 플레이스홀더 데이터 사용)</p>
                </div>
              ) : (
                <Textarea
                  value={blogContent}
            ...
            <CardFooter className="flex flex-col sm:flex-row justify-between gap-3 pt-6">
              ...
              <Button
                ...
                disabled={isLoadingDraft || !blogContent || isLoadingElaboration}
                variant="outline"
         ...
         <Alert className="mt-8 bg-secondary/50 border-primary/30">
            ...
            <AlertTitle className="font-semibold text-primary">AI 지원 기능 및 웹 스크래핑 안내</AlertTitle>
            <AlertDescription className="text-muted-foreground">
              "네이버 뉴스 랭킹 키워드 불러오기"는 현재 서버 측에서 실제 웹 스크래핑 로직 구현이 필요합니다. 
              <span className="font-semibold text-destructive">src/ai/flows/fetch-naver-news-keywords.ts</span> 파일 내의 주석을 참고하여 <code className="bg-muted px-1 py-0.5 rounded">axios</code>, <code className="bg-muted px-1 py-0.5 rounded">cheerio</code> 라이브러리 (설치 필요: <code className="bg-muted px-1 py-0.5 rounded">npm install axios cheerio</code>)를 사용한 실제 스크래핑 코드를 작성해주세요.
              <br/>
